...
- win-builder, R devel (2026-09-21 r90579): the same single note as release, word for word. Examples 25 s, tests 64 s, vignettes and both manuals fine. The devel queue was ten deep, which is why it took four hours rather than fifteen minutes; a re-upload was refused with FTP 550 because the original was still sitting in it.
- R-hub v2, dispatched from the repository's own workflow: `linux` and `windows` on R devel, plus the `donttest`, `atlas` and `mkl` containers, all clean. `macos` failed the same way the GitHub Actions macOS R-devel job does, before reaching the package: `s2`, which `sf` needs, spent eleven minutes compiling abseil-cpp from source and then failed, there being no macOS binaries for R-devel. Two independent systems reaching the same wall is the evidence that it is the platform and not the package. `atlas` and `mkl` were chosen on purpose although the package has no compiled code, because it sums through BLAS in the CD test and two of this week's failures came from BLAS reassociation. The `nosuggests` container errors in one place only, re-building the vignettes without `rmarkdown`; its examples and tests pass, so nothing in the package reaches for a suggested package unguarded.
- Next: the maintainer submits.

## 2026-09-24, session 11 (the presentation layer)

- Done: the visual system written in the parallel Codex session goes into 0.1.0 at the maintainer's decision. `lamp_theme()`, `lamp_colours()` and `lamp_table()` are exported, `lamp_table()` has six methods, every `plot()` method draws through the same theme and palette, and `inst/templates/report.css` styles the HTML report; all eight plot baselines, four vignettes and two figures were regenerated to match. Reviewed before committing, which turned up one thing: `lamp_theme()` sets `legend.location`, a ggplot2 3.5.0 element, and `ggplot2` carried no version bound at all, so an older ggplot2 would have failed at the first plot. `Imports` now says `ggplot2 (>= 3.5.0)`.
- Check: 0 errors, 0 warnings, 1 note ("New submission"). 1,245 tests pass, 0 warnings; examples 30 s, tests 131 s, vignettes rebuild clean; installed size 4.2 MB. styler, lintr and spelling clean.
- win-builder, R devel (2026-09-21 r90579) on this tree: 1 note, "New submission" plus the same two words in `DESCRIPTION`. Examples 26 s, tests 67 s, vignettes and both manuals fine. Nothing the presentation layer added shows up anywhere in the log.
- Next: read the R release log for this tree, then the maintainer submits.
